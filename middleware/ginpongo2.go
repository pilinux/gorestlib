...
func Pongo2(baseDirectory string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer func() {
			if r := recover(); r != nil {
				log.Error("panic msg: gorestlib-middleware-pongo2 panicked")
			}
		}()
		...
		if err != nil {
			log.WithError(err).Panic("panic msg: gorestlib-middleware-pongo2 failed to create a new LocalFilesystemLoader")
		...
		if err := fs.SetBaseDir(s.Globals["base_directory"].(string)); err != nil {
			log.WithError(err).Panic("panic msg: gorestlib-middleware-pongo2 failed to set base directory")
			http.Error(c.Writer, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			log.WithError(err).Panic("panic msg: gorestlib-middleware-pongo2 base directory not found")
			http.Error(c.Writer, err.Error(), http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			log.WithError(err).Panic("panic msg: gorestlib-middleware-pongo2 failed to execute the template with the given context")
			http.Error(c.Writer, err.Error(), http.StatusInternalServerError)
			return
		}
	}
